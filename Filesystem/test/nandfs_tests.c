...
int check_all_pages_in_block(int block, char c) {
    ...
    for (int i = 0; i < NUM_PAGES_PER_BLOCK; i++) {
        ...
        NAND_ReturnType ret = NAND_Page_Read(&addr, PAGE_LEN, (uint8_t *) buffer);
        if (ret != Ret_Success) {
            printf("read b %d p %d r %d\r\n", block, i, ret);
            return ret;
        }
        for (int j = 0; j < PAGE_LEN; j++) {
            if (buffer[j] != c) {
                printf("buff b %d p %d %d != %d\r\n", block, i, buffer[j], c);
                return -1;
            }
        }
    }
...
int pattern_with_filesystem_test(int page_cnt) {
    ...
    if ((rc = NANDfs_close(fd))) {
        printf("close file %d failed: %d\r\n", file_id, nand_errno);
    }

    printf("wrote file id %d\r\n", file_id);

    fd = NANDfs_open_latest();
    if (!fd) {
        printf("open file %d failed: %d\r\n", file_id, nand_errno);
        return -1;
    }
    ...
    for (count = 0; count < page_cnt; count++) {
        ...
        for (int i = 0; i < PAGE_DATA_SIZE; i++) {
            if (page[i] != count && !error_reported) {
                printf("page %d: bad byte %x at offset %d\r\n", count, page[i], i);
                break;
            }
        }
    }
